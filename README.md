...
- Node.js + Express as the backend bridge
- **Bare runtime**, which hosts the QVAC AI worker (`bare-server.js`), separate from the Node/Express process

## API & Network Disclosure

See [`api-disclosure.json`](./api-disclosure.json) for a structured, machine-readable breakdown of all network activity. In short: **zero remote AI inference calls**. All inference runs on-device via QVAC. The only network activity is a one-time model download from Hugging Face on first run.

See [`performance-log.csv`](./performance-log.csv) for a structured log of model load times and inference call durations captured during a real demo run (generation + continuity check), including RAG ingest/search timing.
...
Wait for Terminal 3 to print `LLM model ready.` and `Watching for prompts...` (first run downloads roughly 1 to 2GB of models, so expect this to take a few minutes). Then open the URL printed by Terminal 1 in your browser.
...
See [`hardware-proof.png`](./hardware-proof.png) for a system profiler screenshot confirming these specs.

All inference runs on **CPU**, not GPU (see below for why). This is a deliberate choice that also means this runs on genuinely constrained, years-old consumer hardware, not a high-end workstation.

**A known, documented tradeoff:** during development, GPU (Vulkan) inference intermittently produced a `vk::Device::waitForFences: ErrorDeviceLost` crash on this hardware/driver combination after a model completion finished. Rather than ship something that could silently fail mid-demo, models are explicitly loaded with `device: "cpu"` in `modelConfig`. This trades inference speed (CPU completions can take 30 to 90+ seconds on this hardware) for reliability. On more capable or better-driver-supported hardware, removing `device: "cpu"` from the `modelConfig` in `bare-server.js` should re-enable GPU acceleration. I leave it CPU-only here for reproducible, crash-free demoing on the hardware I tested on.
...
- I evaluated QVAC's tool-calling support (`completion()` with a `tools` array) using the documented pattern and `QWEN3_1_7B_INST_Q4`. In my testing, the model reasoned about tool availability but did not invoke the tool call mechanism. I'm noting this as an honest finding rather than claiming a capability I couldn't verify working.
